...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_message(context.bot, chat_id,
        "Привет, брат.\n\n"
        "Я буду писать три раза в день - просто напомнить одну важную вещь: сегодня не надо.\n\n"
        "Когда тяжело - жми «✊ Держусь».\n"
        "Все, кто рядом - получат пуш. Увидят, что ты есть и не сдаешься.\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "▶ Начать":
        ...
        await send_message(context.bot, chat_id, random.choice(["Че как оно? Главное держись.", "Держусь.", "Всё по плану?", "Сегодня будет полегче.", "Я рядом."]))
    elif text == "✊ Держусь":
